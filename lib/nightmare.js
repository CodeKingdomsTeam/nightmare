...
function Nightmare(options) {
  ...
  // This handler causes problems as it prevents mocha from terminating nicely. The process is ended on exit anyway
...
Nightmare.prototype.run = function(fn) {
  ...
  function done (err) {
    if( err ) {
      throw new Error('Error from a nightmare action: ' + err + '\n\nOriginal stack was: ' + current.originalStack);
    }
    self.running = false;
    if (self.ending) {
      endInstance(self);
