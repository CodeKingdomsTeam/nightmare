...
function Nightmare(options) {
    ...
    this.queue((done) => {
        ...
        // TODO: This handler causes problems as it prevents mocha from terminating nicely. The process is ended on exit anyway
    ...
    function done(err) {
        ...
        if (err) {
            ...
            if (err instanceof Error) {
                ...
                printableErr = err.toString() + ' ' + err.stack;
            } else if (typeof err === 'object') {

                printableErr = JSON.stringify(err, null, 2);
            } else {

                printableErr = err;
            }

            doneargs[0] = new Error('Error from a nightmare action: ' + printableErr + '\n\nOriginal stack was: ' + current.originalStack);
        }
        self.running = false;
        if (self.ending) {
...
Nightmare.prototype.evaluate_now = function(js_fn, done) {
    ...
    // Revert babel typeof transform
    var source = template.execute({ src: String(js_fn).replace(/_typeof/g, 'typeof'), args: args });
...
Nightmare.prototype.queue = function(done) {
    ...
    var args = sliced(arguments);
    var fn = args.pop();
    var originalStack = new Error('Original Stack: ').stack;
    this._queue.push({ fn: fn, args: args, originalStack: originalStack });
};
