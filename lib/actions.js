...
exports.wait = function () {
  ...
  if (typeof arg === 'number') {
    ...
    debug('.wait() for ' + arg.toString(), args.slice(1, -1));
    args.unshift(this);
...
function waitfn() {
  ...
  function tick (self, fn/**, arg1, arg2..., done**/) {
    ...
    var waitDone = function (err, result) {
      debug('.wait() done for function', fn, args.slice(2, -1), 'done', err, result);
      if( err ) {
        return done(err);
      }
      if (result) {
        clearTimeout(timeoutTimer);
        clearTimeout(softTimeoutTimer);
        // TODO: Fix this passing through result
        return done();
      } else if(err) {
        clearTimeout(timeoutTimer);
        clearTimeout(softTimeoutTimer);
...
exports.sendInputEvent = function (event, done) {
  ...
  this.child.call('sendInputEvent', event, done);
  return this;
...
exports.clearStorageData = function (options, done) {
  ...
  this.child.call('clearStorageData', options, done);
