  ...
  if (typeof arg === 'number') {
    ...
    if(arg < this.optionWaitTimeout){
      ...
      waitms(this.optionWaitTimeout, function(){
        done(new Error('.wait() ' + arg + ' timed out after '+this.optionWaitTimeout+'msec'));
      }.bind(this));
    }
  }
...
function waitfn (self, fn/**, arg1, arg2..., done**/) {
  ...
  var waitDone = function (err, result) {
    if (result) {
      ...
      _waitMsPassed = 0;
      return done(new Error('.wait() for ' + fn + ' timed out after '+self.optionWaitTimeout+'msec'));
    }
    else {
