...
app.on('ready', function () {
  ...
  parent.respondTo('sendInputEvent', function (event, done) {
  ...
  parent.respondTo('cookie.clear', function (cookies, done) {
    ...
    if (cookies.length == 0) {
      getCookies = (cb) => win.webContents.session.cookies.get({
        url: url
      }, (error, cookies) => {
   ...
   * Clear Storage Data
   */
  parent.respondTo('clearStorageData', function (options, done) {
    parent.emit('log', 'clearStorageData', options);
    win.webContents.session.clearStorageData(options, function (error) {
      if (error) return done(error);
      parent.emit('log', 'clearStorageData done', options);
      done();
    });
  });
